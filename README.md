...
| `dac <on [slpdat]\|off>` | the DAC2 triangle. `slpdat` is the step per DAC clock, so **larger is faster**; 64 puts a full period in one captured buffer |
| `dactest [halves]` | the DAC test on its own |
...
| `dac.c`, `dac.h` | DAC2 in Triangle Wave mode (CLKGEN7 as its clock) and the `UREFCON` routing that puts it on the chip's internal UREF line, where every ADC core can sample it as `ANn7` — the known signal for the DAC test, with no pin and no wire |
| `dactest.c`, `dactest.h` | captures one contiguous buffer with the stream stopped from the DMA interrupt, then judges it: a changing signal, even steps, no jump, and the triangle period measured from the data. `[dactest]` lines, PASS/FAIL |
