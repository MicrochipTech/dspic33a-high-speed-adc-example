 ...
 * bit 15. The trigger to the ADC is the period rollover, put on the
 * module's auxiliary output with AUXOUT = 01 (Table 26-10) - without
 * that the ADC sees nothing, whatever TRG1SRC/TRG2SRC select.
 *
...
void sccp1_start(uint32_t ticks)
{
    ...
    CCP1CON2 = 0u;
    /* The signal the ADC sees as "SCCP1 trigger" is the module's
     * auxiliary output, and with AUXOUT = 00 there is none (Table
     * 26-10, p1818). 01 in timer mode = "Time Base Period Reset or
     * Rollover": one pulse per period. Runs 5 and 6 on the board had
     * AUXOUT = 00 - the SCCP1 candidate never emitted a trigger. */
    CCP1CON2bits.AUXOUT = 1u;
